...
- **4.1 Multi-difficulty suite.** *Independent.* Add at least two more graduated problems at higher difficulty (e.g., `002_*` at difficulty 2, `003_*` at difficulty 3). TDD: suite loader yields all problems, sorted/groupable by difficulty. **Update Phase 2.5 and Phase 3.5 acceptance tests to filter to a stable single-problem subset** so they don't iterate the whole suite on every real-Pi run. *(See open spike: 0011 — acceptance test tiering; the problem-filter footgun is addressed in 3.5.1.)*

- **4.2 Per-(problem, metric) events.** *Depends on Phase 2 ScoringPort.* Each trial's `events.jsonl` records one event per `(problem, metric)` pair, not aggregated. **Per-pair payload carries `(value, n_samples)` per ADR 0006** — `n_samples=1` under the single-shot default, `n_samples > 1` only when `replicates > 1`. Cost is two values per trial (`tokens`, `dollars`) per ADR 0005. The lifecycle event phases from ADR 0007 (`error_retry`, `error_escalated`, `boundary_violation`) ride alongside the per-(problem, metric) events. *(See open spike: 0008 — trial-outcome classification single source of truth; the new lifecycle event phases land here and need a coordinated classifier.)* TDD: a multi-problem trial writes events for every problem; `replicates > 1` runs accumulate matching `n_samples`; a boundary-violated trial emits the matching lifecycle phase.
...
| --- | --- | --- | --- |
| 0008 (planned) | **Trial-outcome classification — single source of truth.** The cost-cap watchdog in `run_trial` sets `boundary_violation` directly, bypassing `_classify_outcome`. As more boundary triggers (timeout) and lifecycle event phases (`error_retry`, `error_escalated`, `boundary_violation`) land, who owns the final outcome — an inline watchdog override, or a single event-stream-driven classifier reading the trial's emitted phases? | Phase 4.2 | Open |
...
| 0011 (planned) | **Acceptance-test tiering for real-Pi cost.** Phase 3.5 took ~7m51s for 2 tests at 4 trials × 1 problem. Phase 4.1 multiplies by `n_problems`, 4.2 by `replicates`, 6 by trial count for surrogate quality. Split into `acceptance_fast` (1 trial, retry-budget 0, single problem) and `acceptance_full` (production defaults), or accept that real-Pi acceptance is CI-occasional and document the budget? | Phase 4.1 | Open |
