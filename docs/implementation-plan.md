...
- **4.1 Multi-difficulty suite.** *Independent.* Add at least two more graduated problems at higher difficulty (e.g., `002_*` at difficulty 2, `003_*` at difficulty 3). TDD: suite loader yields all problems, sorted/groupable by difficulty. **Update Phase 2.5 acceptance test to pin `problem_ids=["001_binary_search"]`** — Step 3.5.1 only pinned Phase 3.5; Phase 2.5 is still unfiltered and will silently iterate every new problem once 002+ land. **Phase-entry condition (ADR 0010): Phase 4's acceptance test must ship `acceptance_fast` and `acceptance_full` variants** — the fast variant is the dev/CI signal, the full variant runs once to prove production-fidelity behavior. *(The filter itself was hardened in commits `9cf50c3` + `0df4810`: pre-`from_dict` allowlist, bare-`str` rejection, documented `[]` vs `None` semantics.)*
