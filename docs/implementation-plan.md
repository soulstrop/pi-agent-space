...
**Doc-sync gate for cleanup commits.** When a cleanup/extraction commit deletes or renames a symbol referenced in `docs/design-notes.md`, `docs/implementation-plan.md`, ADRs, or `REVIEW.md`, the same commit (or an immediate sibling) updates those references. Stale "Where:" pointers and resolution-justification prose actively mislead future contributors — per the design-notes "consult before unwinding" rule, the breadcrumb must keep up with the code.
...
- `GraduatedProblemSetAdapter` loads every `problem.json` in its base dir. **Step 3.5.1 (commit `9cf50c3`) added a `problem_ids: Iterable[str] | None` allowlist parameter** so single-problem acceptance tests can pin to a stable subset before Phase 4.1 lands 002+ problems. Phase 3.5's acceptance test now pins to `["001_binary_search"]`. **Filter caveats:** matches the JSON `id` field (not the directory name); a bare `str` is interpreted character-wise and silently empties the result; `problem_ids=[]` is a load-nothing allowlist (not "no filter"). The allowlist check runs *after* `GraduatedProblem.from_dict`, so a malformed `problem.json` in a non-allowlisted directory still aborts `load()` — see Phase 4.1's filter-hardening prerequisite.
...
- **3.5.1 Phase 3 cleanup / sync.** *Post-Phase-3 tidying — low impact, derisks Phase 4. **Completed.*** Three small fixes surfaced by the Phase 3.4/3.5 retrospective:
    - **Extracted the model-error predicate** into `pi_evaluator/lifecycle.py` (commit `2611d45`). Previously the rule lived twice — `trial_runner._has_model_error` and `cli_subprocess_adapter._is_retryable_error`. Phase 4.2's new lifecycle event phases need a single source; see Phase 4.2's scoping note below.
    - **Added `problem_ids: Iterable[str] | None` filter to `GraduatedProblemSetAdapter`** (commit `9cf50c3`); Phase 3.5 acceptance test now pins to `["001_binary_search"]`. Phase 2.5 was *not* updated in 3.5.1 — see Phase 4.1's prerequisite.
    - **Documented `OptimizerDriver.retry_budget` as declarative-only in v1** (commit `cfa9f53`). The driver param does not flow into the harness; `CliSubprocessAdapter` is the actual configuration point. Resolution (remove or plumb-through) defers to v2 — tracked under "What's deferred."
...
- **4.1 Multi-difficulty suite.** *Independent.* Add at least two more graduated problems at higher difficulty (e.g., `002_*` at difficulty 2, `003_*` at difficulty 3). TDD: suite loader yields all problems, sorted/groupable by difficulty. **Update Phase 2.5 acceptance test to pin `problem_ids=["001_binary_search"]`** — Step 3.5.1 only pinned Phase 3.5; Phase 2.5 is still unfiltered and will silently iterate every new problem once 002+ land. **Hard precondition: spike 0011 must close before 4.1 begins** — the 7m51s × `n_problems` cost ceiling makes the fast-tier acceptance-test split a blocker, not a sidebar. *(The filter itself was hardened in commits `9cf50c3` + `0df4810`: pre-`from_dict` allowlist, bare-`str` rejection, documented `[]` vs `None` semantics.)*

- **4.2 Per-(problem, metric) events.** *Depends on Phase 2 ScoringPort.* Each trial's `events.jsonl` records one event per `(problem, metric)` pair, not aggregated. **Per-pair payload carries `(value, n_samples)` per ADR 0006** — `n_samples=1` under the single-shot default, `n_samples > 1` only when `replicates > 1`. Cost is two values per trial (`tokens`, `dollars`) per ADR 0005. The lifecycle event phases from ADR 0007 (`error_retry`, `error_escalated`, `boundary_violation`) ride alongside the per-(problem, metric) events. **Hard precondition: spike 0008 must close before 4.2 begins** — without a single-source classifier the new phases compound the watchdog-vs-classifier ambiguity that 0008 was opened to resolve. **lifecycle.py scoping rule (from Step 3.5.1's docstring):** the module owns *telemetry-classified* outcomes only (predicates over `RawTelemetry`). `boundary_violation` is *watchdog-classified* per ADR 0007 A2 and stays with its killer (`TrialRunner.run_trial` for cost cap, future timeout). Phase 4.2 grows `lifecycle.is_model_error` (or splits it) for `error_retry` / `error_escalated` only. TDD: a multi-problem trial writes events for every problem; `replicates > 1` runs accumulate matching `n_samples`; a boundary-violated trial emits the matching lifecycle phase from its watchdog, not from `lifecycle.py`.
...
- **6.2 Surrogate model.** *Depends on 6.1.* **Heteroscedastic Gaussian Process per ADR 0006** — `HeteroskedasticSingleTaskGP` from BoTorch (or equivalent) over the feature vector predicting the 4D output (5D with subjective). The HetGP models input-dependent variance; the trial event stream's `(value, n_samples)` payloads from Phase 4.2 feed the noise model. **Bootstrap discipline** per ADR 0006: below ~10 trials the surrogate is unreliable, so acquisition (Step 6.3) ignores its mean estimate. *(See open spike: 0012 — boundary-violated trials are filtered off the Pareto frontier but ADR 0006 wants the surrogate to see them as cliff signal. 6.2 needs two views over the same trial set.)* TDD: trained on a known function, the surrogate predicts within tolerance on held-out points and reports calibrated variance estimates; bootstrap behavior under sparse training data matches spec.
...
- **`OptimizerDriver` no-op kwargs.** Step 3.5.1's commit `cfa9f53` documented `retry_budget` as declarative-only in v1; the driver stores it but does not plumb it into the harness. Asymmetric with `replicates`, which raises `NotImplementedError` for non-default values. Resolve in v2: either plumb through `TrialRunner` → `CliSubprocessAdapter`, or drop the kwarg. Mirror the resolution in any future no-op kwargs the driver collects.
...
| --- | --- | --- | --- |
| 0008 (planned) | **Trial-outcome classification — single source of truth.** The cost-cap watchdog in `run_trial` sets `boundary_violation` directly, bypassing `_classify_outcome`. As more boundary triggers (subprocess timeout, **sandbox-kill** from ADR 0009 bwrap resource limits — currently swallowed by `lifecycle.is_model_error`'s `exit_code != 0` rule, which misattributes infrastructure kills as model errors) and lifecycle event phases (`error_retry`, `error_escalated`, `boundary_violation`) land, who owns the final outcome — an inline watchdog override, or a single event-stream-driven classifier reading the trial's emitted phases? | Phase 4.2 (blocks 4.2) | Open |
| 0009 (planned) | **Driver-run event log.** Per-run concerns (cost-cap warning, circuit-breaker trips, future async subjective scoring) don't fit the trial event stream because they cross trial boundaries. Today the per-run cost-cap warning routes through Python `logging`. Add a structured driver-scope event log (e.g., `trials/run.jsonl`), or commit to `logging` and stop pretending events are the only durable trace? | Phase 5.2 | Open |
| 0010 (planned) | **Re-finalize semantics and the `finalized`-is-last invariant.** ADR 0003 does not pin whether `finalized` must be the last entry in `events.jsonl`. The Phase 3.4 design-note asserts it informally to justify a routing choice. Phase 5.2's atomic re-finalize for retroactive subjective scoring needs to either honor the invariant (and use a successor event phase like `re_finalized`) or break it (and pin the new ordering rule explicitly). | Phase 5.2 | Open |
| 0011 (planned) | **Acceptance-test tiering for real-Pi cost.** Phase 3.5 took ~7m51s for 2 tests at 4 trials × 1 problem. Phase 4.1 multiplies by `n_problems`, 4.2 by `replicates`, 6 by trial count for surrogate quality. Split into `acceptance_fast` (1 trial, retry-budget 0, single problem) and `acceptance_full` (production defaults), or accept that real-Pi acceptance is CI-occasional and document the budget? | Phase 4.1 (blocks 4.1) | Open |
| 0012 (planned) | **Boundary-violated trials: frontier vs. surrogate visibility.** Boundary-violated trials carry `quality_score=0` and so are always dominated by any completed trial — they get filtered off the Pareto frontier. ADR 0006 wants the HetGP to see them as cliff signal in feature space. Phase 6.2 has two consumers (frontier UX, surrogate fit) reading the same trial set with different filters. Pin the two views explicitly in the surrogate-training pipeline, or expose a separate `optimizer_training_set()` accessor? | Phase 6.2 | Open |
