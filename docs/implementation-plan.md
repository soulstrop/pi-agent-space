...
- **3.5.1 Phase 3 cleanup / sync.** *Post-Phase-3 tidying — low impact, derisks Phase 4.* Three small fixes surfaced by the Phase 3.4/3.5 retrospective:
    - **Extract the model-error predicate** into a shared module (e.g., `pi_evaluator/lifecycle.py`). Today the rule lives twice — `trial_runner._has_model_error` and `cli_subprocess_adapter._is_retryable_error` — and both must stay in sync as ADR 0007 evolves. Phase 4.2's new lifecycle event phases (`error_retry`, `error_escalated`, `boundary_violation`) will compound the drift risk if left.
    - **Add a `problem_ids: list[str] | None` filter to `GraduatedProblemSetAdapter`** and have the Phase 3.5 acceptance test pass `["001_binary_search"]` explicitly. Phase 3.5 currently works only because `002_*` doesn't exist yet; the filter derisks Phase 4.1's multi-problem expansion and removes the silent-iteration footgun.
    - **Document `OptimizerDriver.retry_budget` as declarative-only in v1.** The driver param does not flow into the harness; `CliSubprocessAdapter` is the actual configuration point. Update the driver's docstring + `__init__` parameter doc to make the divergence explicit. Resolution (remove or plumb-through) defers to v2.

  TDD: predicate-extraction round-trip; filter-respects-allowlist with a 2-problem fixture; the docstring update is a non-test change verified by reading.
