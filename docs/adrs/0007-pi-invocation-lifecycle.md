...
- Three new trial event phases (`error_retry`, `error_escalated`, `boundary_violation`) join the existing phase set (`configured`, `eval`, `scored_objective`, `scored_subjective`, `finalized`). The `finalized` event payload gains an `outcome` field; absent on existing trials it defaults to `"completed"` for backward compatibility. (ADR 0012 later retires `scored_objective` in favor of `metric_record` events; the rest of this list remains as enumerated.)
